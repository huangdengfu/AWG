...
document.getElementById("showInfoButton").addEventListener("click", function () {
  ...
  loadCSVAndDisplay('AWG美规线号.csv'); // 加载CSV文件
});
...
function loadCSVAndDisplay(url) {
  ...
  fetch(url)
    ...
    .then(csv => {
      ...
      rows.forEach(row => {
        const cells = row.split(",");  // 按逗号分割单元格
        table += "<tr>";
        cells.forEach(cell => {
          table += `<td style='padding: 8px; text-align: left;'>${cell}</td>`;
        });
        table += "</tr>";
      });
      ...
      // 将生成的表格插入到弹窗内容区域
      document.getElementById("csvContentContainer").innerHTML = table;
    })
    .catch(error => {
      // 如果加载出错，显示错误信息
      document.getElementById("loadingMessage").style.display = "none";
      document.getElementById("csvContentContainer").innerHTML = "<p style='color: red;'>加载CSV文件时出错。</p>";
